...
# -*- coding: utf-8 -*-
"""
Created on Mon Dic 02 13:21:04 2024
...
class WriteSplitRandomFile(FileWriter):
    def save(self,results_path,locations,**kwargs):
        ...
        fname = f'{results_path}SensorsLocations_Boyd_N{n}_S{signal_sparsity}_VarThreshold{variance_threshold_ratio}_nSensors{n_locations_monitored}_randomSeed{random_seed}_split{rois_split}_subsplitSeed{seed_subsplit}.pkl'
        with open(fname,'wb') as f:
            pickle.dump(locations,f,protocol=pickle.HIGHEST_PROTOCOL)
        print(f'File saved in {fname}')
...
def iterative_placement(algorithm,Psi,n,signal_sparsity,roi_idx,variance_threshold_ratio,fixed_n_sensors=-1):
    """
    Wrapper for sensor sizing and placement algorithm.
    The number of sensors and their locations are determinined iteratively
    by reducing the number of deployed sensors at each step.
    
    The algorithm stops once the problem becomes unfeasible.
    There might be different sensor placement problems.

    Args:
        algorithm (_type_): _description_
        Psi (_type_): _description_
        n (_type_): _description_
        signal_sparsity (_type_): _description_
        roi_idx (_type_): _description_
        variance_threshold_ratio (_type_): _description_
        fixed_n_sensors (int, optional): _description_. Defaults to -1.
    ...
    # sensor placement for a fixed number of sensors      
    if fixed_n_sensors > 0:
        sensor_placement = sp.SensorPlacement(algorithm, n, signal_sparsity,
                                            n_refst=fixed_n_sensors,n_lcs=0,n_unmonitored=n-fixed_n_sensors)
        sensor_placement.initialize_problem(Psi,rho=rho)
        sensor_placement.solve()
        sensor_placement.discretize_solution()
        sensor_placement.C_matrix()
    ...
    # iterative wrapper until number of sensors makes the problem unfeasible
    else:
        for n_sensors in np.arange(n,signal_sparsity-1,-1):
            covariance_matrix_fullymonitored = Psi@np.linalg.inv(Psi.T@Psi)@Psi.T
            sensor_placement = sp.SensorPlacement(algorithm, n, signal_sparsity,
            ...
            covariance_matrix = Psi@np.linalg.inv(Psi.T@sensor_placement.C[1].T@sensor_placement.C[1]@Psi)@Psi.T
            worst_coordinate_variance_fullymonitored_roi = [np.diag(covariance_matrix_fullymonitored)[idx].max() for idx in roi_idx.values()]
            worst_coordinate_variance_roi = [np.diag(covariance_matrix)[idx].max() for idx in roi_idx.values()]
            threshold = [i*j for i,j in zip(variance_threshold_ratio,worst_coordinate_variance_fullymonitored_roi)]
            if np.sum([i<=j for i,j in zip(worst_coordinate_variance_roi,threshold)])!=len(threshold):
                print(f'Design thresholds not met for {n_sensors} sensors.')
                sensor_placement = previous_problem
                break
            elif sensor_placement.problem.status != 'optimal':
                print(f'Unfeasible problem for {n_sensors} sensors.')
                sensor_placement = previous_problem
                break
            else:
                previous_problem = sensor_placement
        return sensor_placement

        
def simple_placement(Psi,n,signal_sparsity,n_sensors):
...
def Joshi_Boyd_VarianceThreshold(roi_idx:dict,roi_threshold:list,variance_threshold_ratio:list,snapshots_matrix_train_centered:np.ndarray,reverse_roi_order:bool,force_n:int=-1,algorithm = 'JB_trace'):
    """
...
if __name__ == '__main__':
    ...
    if sensor_placement_rois:
        ...
        if num_rois == 2:
            random_seed = 0
            roi = ROI(RandomRoi())#[RandomRoi,SplitRandomRoi]
            ...
            seed_subsplit = 0
            rois_split = [0]
            roi = ROI(SplitRandomRoi())#[RandomRoi,SplitRandomRoi]
        ...
        algorithm = 'JB_feasibility_diagonal'#['JB_feasibility_program','JB_feasibility_diagonal']
        sensor_placement = iterative_placement(algorithm,Psi,
                                               n,signal_sparsity,
                                               roi_idx,variance_threshold_ratio,
                                               fixed_n_sensors=-1)
        locations = [sensor_placement.locations[1],[i for i in np.arange(n) if i not in sensor_placement.locations[1]]]
        
        # deploy sensors and compute variance
        sensor_placement.C_matrix()
        cov_matrix = Psi@np.linalg.inv(Psi.T@sensor_placement.C[1].T@sensor_placement.C[1]@Psi)@Psi.T
        worst_coordinate_variance = np.diag(Psi@np.linalg.inv(Psi.T@sensor_placement.C[1].T@sensor_placement.C[1]@Psi)@Psi.T).max()
        locations_monitored = sensor_placement.locations[1]
        n_locations_monitored = len(locations[0])
        n_locations_unmonitored = len(locations[1])
        ...
        # save results
        if num_rois ==2:
            saver = SaveLocations(WriteRandomFile())#[WriteRandomFile,WriteSplitRandomFile]
            saver.save_locations(results_path,locations[0],n=n,signal_sparsity=signal_sparsity,variance_threshold_ratio=variance_threshold_ratio,
                                n_locations_monitored=n_locations_monitored,seed=random_seed)
  
        elif num_rois == 3:
            saver = SaveLocations(WriteSplitRandomFile())#[WriteRandomFile,WriteSplitRandomFile]
            saver.save_locations(results_path,locations[0],n=n,signal_sparsity=signal_sparsity,variance_threshold_ratio=variance_threshold_ratio,
