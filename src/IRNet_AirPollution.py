...
# dataset
class Dataset():
    def __init__(self,pollutant:str='O3',start_date:str='2011-01-01',end_date:str='2022-12-31',files_path:str='',synthetic_dataset:bool=False):
...
class Figures():
    ...
    def geographical_network_visualization(self,map_path:str,coords_path:str,locations_monitored:np.array=[],show_legend:bool=False,synthetic_dataset:bool=False,save_fig:bool=False):
    ...
    def boxplot_validation_rmse_svd(self,rmse_sparsity,max_sparsity_show=10,synthetic_dataset=False,save_fig=False) -> plt.figure:
    ...
    def curve_errorvariance_comparison(self,errorvar_fullymonitored:list,errorvar_reconstruction:list,variance_threshold_ratio:float,worst_coordinate_variance_fullymonitored:float,n:int,n_sensors:int,errorvar_reconstruction_Dopt:list=[],save_fig:bool=False) -> plt.figure:
        ...
        ax.plot(errorvar_reconstruction,color='#1a5276',label=f'IRNet solution')
...
if __name__ == '__main__':
    ...
    dataset = Dataset(pollutant,start_date,end_date,files_path,synthetic_dataset)
    ...
    if determine_sparsity:
        ...
        fig_rmse_sparsity_val = plots.boxplot_validation_rmse_svd(rmse_sparsity_val,max_sparsity_show=sing_vals.shape[0],synthetic_dataset=synthetic_dataset,save_fig=False)
        plt.show()
        sys.exit()
    ...
    if deploy_sensors:
        ...
        If epsilon = 1e-1 and n_it = 20:
            When 40 locations have been assigned as:
            28 Locations monitored: [6, 8, 9, 14, 31, 32, 37, 42, 43, 36, 12, 28, 23, 15, 26, 10, 18, 34, 17, 41, 7, 21, 3, 4, 33, 5, 27, 35]
            12 Locations unmonitored: [38, 39, 11, 19, 24, 20, 0, 2, 13, 25, 40, 29]
            Problem problem status is infeasible at next iteration
        
        But If epsilon = 1e-2 and n_it = 20:
            The problem finishes with
            32 Locations monitored: [14, 31, 9, 42, 32, 37, 43, 6, 8, 12, 28, 23, 36, 15, 10, 18, 17, 7, 34, 41, 3, 21, 26, 4, 33, 5, 27, 35, 30, 16, 1, 29]
            12 Locations unmonitored: [0, 20, 24, 25, 40, 2, 11, 13, 19, 38, 39, 22]

        i.e. with epsilon=1e-1 assigned location 29 as unmonitored and then cannot converge. With epsilon=1e-2 assigned location 29 as monitored, although is the last one to enter.
        """
        epsilon = 1e-2
        n_it = 20
        h_prev = np.zeros(n)
