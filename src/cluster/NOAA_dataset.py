...
def recover_map(y:np.array,idx_nan:np.array,n_rows:int,n_cols:int)->np.ndarray:
    ...
    print(f'Length of arary of measurements: {len(y)}. Length of array of NaN indices: {len(idx_nan)}. Length of reconstructed array: {len(X_vect)}\nFirst Nan entry: {idx_nan[0]}. Last Nan entry: {idx_nan[-1]}')
    ...
    print(f'Reconstructed image has dimensions {X_mat.shape}')
    return X_mat
...
def create_dataframe(rootgrp,preprocessing:str='None',downsample:bool=False,scale_factor_x:float=0.1,scale_factor_y:float=0.1,windowing:bool=False,lon_min:float=130.,lon_max:float=230,lat_min:float=40,lat_max:float=140,lat_range:list=[],lon_range:list=[],save_dataset:bool=False)->pd.DataFrame:
    ...
    if preprocessing not in ['downsample','windowing','None']:
        raise ValueError(f'Specify a proper image preprocessing technique. Specified: {preprocessing}')
    ...
    plots.SST_map(data_scaled,show_coords=False,save_fig=False,save_fig_fname='SST_map_scaled.png')

    print(f'Scaled data has dimensions {data_scaled.shape}. Mask of land values has dimensions: {mask_scaled.shape}')
    ...
    if preprocessing == 'None':
        if save_dataset:
            fname = f'{file_path}SST_month_N{snapshots_matrix.shape[0]}.npy'
            np.save(fname,snapshots_matrix)
            np.savetxt(f'{file_path}idx_measurements_N{snapshots_matrix.shape[0]}.csv',idx_measurement,delimiter=',')
            np.savetxt(f'{file_path}idx_land_N{snapshots_matrix.shape[0]}.csv',idx_land,delimiter=',')
            print(f'Files saved in dataset in: {fname}')

        return snapshots_matrix, idx_land

    df = pd.DataFrame(snapshots_matrix.T,dtype=np.float32,columns=idx_measurement,index=date_range)
    df.columns = df.columns.astype(str)
    if save_dataset:
        fname = f'{file_path}SST_month_N{df.shape[1]}.parquet'
        df.to_parquet(fname)
        np.savetxt(f'{file_path}idx_measurements_N{df.shape[1]}.csv',idx_measurement,delimiter=',')
        np.savetxt(f'{file_path}idx_land_N{df.shape[1]}.csv',idx_land,delimiter=',')
        print(f'Files saved in dataset in: {fname}')
    return df, idx_land
...
if __name__=='__main__':
    ...
    fname = f'{file_path}latitudes_range.npy'
    np.save(fname,lat_range)
    fname = f'{file_path}longitudes_range.npy'
    np.save(fname,lon_range)
    ...
    mask = rootgrp.variables['sst'][0].mask
    data[mask] = np.nan
    plots.SST_map(data,show_coords=True,save_fig=False,save_fig_fname='SST_map.png')
    ...
    new_px_x = 100#[40,100]
    new_px_y = 100#[40,100]
    scale_factor_x = new_px_x/data.shape[1]
    scale_factor_y = new_px_y/data.shape[0]
    print(f'Original images have size:\n -num rows: {data.shape[0]}\n -num columns: {data.shape[1]}')
    print(f'Total number of pixels: {n_pixels}\nNumber of pixels with land: {n_land}\nNumber of pixels with measurements in original image: {n_pixels - n_land}')
    ...
    downsampling = False
    if downsampling:
        df,idx_land = create_dataframe(rootgrp,preprocessing='downsample',
                              scale_factor_x=scale_factor_x,scale_factor_y=scale_factor_y,              
                              save_dataset=False)
    no_scaling = True
    if no_scaling:                          
        #df,idx_land = create_dataframe(rootgrp,preprocessing='None',save_dataset=True)
        snapshots_matrix,idx_land = create_dataframe(rootgrp,preprocessing='None',save_dataset=True)
        print(f'Snapshots matrix size: rows: {snapshots_matrix.shape[0]}, column: {snapshots_matrix.shape[1]}')

    print(f'Genrated dataset has {df.shape[0]} measurements for {df.shape[1]} locations')

    print('Testing recovering image from vectorized array without nans.')
    X_recovered = recover_map(df.iloc[0,:].astype(float),idx_land,n_rows=new_px_y,n_cols=new_px_x)    
    plots.SST_map(X_recovered,show_coords=False,save_fig=True,save_fig_fname='SST_map_reconstructed.png')
    plt.show()
