...
class SensorPlacement:
    ...
    def networkPlanning_singleclass_iterative(self,Psi:np.ndarray,rho:float,w:np.array,locations_monitored:list=[],locations_unmonitored:list=[]):
        """
    ...
    def IRL1_networkDesign(self,Psi:np.ndarray,rho:float,w:np.array,locations_monitored:list=[],locations_unmonitored:list=[],locations_forbidden=[],epsilon:float=1e-2,primal_start={'x':[],'sl':[],'ss':[]}):
        """
        Network design algorithm using Candes IRL1 objective function and Rusu method to ensure binary solution.
        The lists must be updated iteratively when calling this method from the solution h.
        ...
        vector_ineq = matrix([matrix(np.tile(0,self.n)),
                              ...
                              matrix(np.tile(epsilon,len(locations_unmonitored))),
    ...
    def IRNet_ROIs(self,Psi:np.ndarray,rho:float,w:np.array,locations_monitored:list=[],locations_unmonitored:list=[],epsilon:float=1e-2,primal_start={'x':[],'sl':[],'ss':[]}):
        """
        Network design algorithm using Candes IRL1 objective function and Rusu method to ensure binary solution.
...
# =============================================================================
    ...
    def initialize_problem(self,Psi:np.ndarray,alpha:float=0.,rho:float=100.,w = cp.Variable(),locations_monitored:list = [],locations_unmonitored:list = [],epsilon:float=1e-2,primal_start:dict={}):
        """
        ...
        if self.algorithm not in algorithms:
            ...
            self.IRNet_ROIs(Psi,rho,w,locations_monitored,locations_unmonitored,epsilon,primal_start)
