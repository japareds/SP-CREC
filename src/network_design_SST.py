...
if __name__ == '__main__':
    ...
    if args.determine_sparsity:
        ...
        # get signal sparsity fvrom either reconstruction threshold or cumulative energy
        rmse_threshold = 0.5
        ...
        signal_sparsity_energy = np.argwhere(cumulative_energy>=energy_threshold)[0][0]
        print(f'Signal sparsity from cumulative energy threshold at {energy_threshold} is {signal_sparsity_energy}. Singular values ratio: {sing_vals[int(signal_sparsity_energy)]/sing_vals[0]:.2e}. Cumulative energy: {cumulative_energy[int(signal_sparsity_energy)]:.2f}')
    ...
    if args.design_large_networks:
        ...
        del U, snapshots_matrix_train, X_train
        deployed_network_variance_threshold = args.variance_threshold_ratio*fully_monitored_network_max_variance
        print(f'Network error variance:\n - Fully monitored network max error variance: {fully_monitored_network_max_variance:.2f}\n - Design error variance ratio: {args.variance_threshold_ratio:.2f}\n - Desired network max error variance: {deployed_network_variance_threshold:.2f}')
        sys.stdout.flush()
        ...
        # IRNet function for large-scale deployments
